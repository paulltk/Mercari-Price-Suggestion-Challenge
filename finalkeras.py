...
def make_sparse_matrix(data, description_dict, categories_dict, brand_dict):
    sparse_matrix = sparse.lil_matrix(
        (data.shape[0], len(description_dict) + len(categories_dict) + len(brand_dict) + 6), dtype=bool)
...
def fit_neuralnet(neuralnet, sparse_matrix, prices, batchsize, amount_batches=1):
    ...
    for t in range(amount_batches):
        ...
        scores.append(calc_score(valbatchprices, predicted_prices))
...
scores = fit_neuralnet(neuralnet, sparse_matrix, prices, 10000, amount_batches=1)
print(scores)
print("neural net fitted:", time.time() - start)

sparse_test_matrix = preprocess_test(test, dicts, start)
print("sparse test matrix made")
predicted_prices = neuralnet.predict(sparse_test_matrix.todense())
print("predicted prices of test matrix")
